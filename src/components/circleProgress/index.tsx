...
const CircleProgress: FC<Props> = ({ width, valuesArr, radius, callback }) => {
  const ref: MutableRefObject<HTMLCanvasElement | undefined> =
    useRef<HTMLCanvasElement>();
  let [index, setIndex] = useState(0);

  const { start, stop } = useIntervalFunc(1000, valuesArr.length, callback);
  ...
  useEffect(() => {
    ...
    const generateFunc = () => {
      const oneTurn = (Math.PI * 2) / valuesArr.length;
      ...
      return () => {
        ...
        drawCircle({
          ...
          r: radius,
          sRadian: -Math.PI / 2,
          eRadian: Math.PI * 1.5 - index * oneTurn,
          color: index <= valuesArr.length / 2 ? "green" : "orange",
        });
        setIndex(index++);
      };
    };
